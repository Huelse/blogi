...
- [ ] 接入 Nuxt 国际化方案，为首页、登录、注册、后台、评论等页面抽离文案
- [ ] 增加语言切换入口，并支持至少中英文两套文案
- [ ] 将 `/admin`、`/admin/posts/*`、`/admin/settings` 的登录校验改为统一路由中间件
- [ ] 统一处理未登录跳转和登录后回跳逻辑，避免在页面内重复编写 `navigateTo`

### M2. 文件上传（头像 / 封面）

- [ ] 新增上传接口，支持 `multipart/form-data`
- [ ] 为文章增加封面上传能力，并在前端编辑器中支持选择、预览和回填
- [ ] 为用户或游客资料增加头像上传能力
- [ ] 设计文件存储策略：本地磁盘或对象存储
- [ ] 增加文件类型、大小、安全校验和访问 URL 规则
- [ ] 为 `posts` 增加 `cover_url` 字段，为 `users` / `visitors` 增加 `avatar_url` 字段

### M3. AI 摘要生成 + Redis 缓存阅读量 / 点赞量

- [ ] 接入 AI 服务，为文章生成摘要
- [ ] 在发文和编辑页面增加“AI 生成摘要”按钮、加载态和失败降级提示
- [ ] 保留当前基于正文截断的摘要逻辑，作为 AI 不可用时的降级方案
- [ ] 增加文章阅读量展示，并使用 Redis 进行计数或缓存
- [ ] 为点赞量增加 Redis 缓存读取与失效策略
- [ ] 设计阅读量去重策略，避免短时间重复刷新造成异常计数
- [ ] 为阅读量持久化补充数据库字段和回写机制
