...
<template>
  <main class="mx-auto max-w-xl px-6 py-16">
    <UiCard class="p-8">
      <p class="text-brand text-sm uppercase tracking-[0.3em]">{{ t('auth.register.eyebrow') }}</p>
      <h1 class="text-title mt-4 text-3xl font-semibold">{{ t('auth.register.title') }}</h1>
      <p class="text-muted mt-3 text-sm leading-7">{{ t('auth.register.description') }}</p>
      ...
      <form class="mt-8 space-y-5" @submit.prevent="submit">
        ...
        <div>
          <UiLabel for="displayName">{{ t('auth.register.displayName') }}</UiLabel>
          <UiInput
            id="displayName"
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <NuxtLink
            ...
            :to="{ path: '/login', query: authEntryQuery }"
          >
